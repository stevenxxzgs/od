...
class DataInput(object):
    ...
    def load_data(self):
        ODPATH = self.data_dir + '/od_day20210710_20210731.npz'
        OD_DAYS = [date.strftime('%Y-%m-%d') for date in pd.date_range(start='2021-07-10', end='2021-07-31', freq='1D')]
        prov_day_data = ss.load_npz(ODPATH)
        prov_day_data_dense = np.array(prov_day_data.todense()).reshape((-1, 2471, 2471))
        data = prov_day_data_dense[-len(OD_DAYS):,1950:2100,1950:2100,np.newaxis]
        ODdata = np.log(data + 1.0)      # log transformation
        print("ODdata.shape: ",ODdata.shape)

        adj = np.load(self.data_dir + '/adjacency_matrix.npy')
        adj = adj[1950:2100,1950:2100]
